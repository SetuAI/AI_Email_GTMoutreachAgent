...
def create_company_finder_agent(db: SqliteDb) -> Agent:
    ...
    return Agent(
        ...
        instructions=[
            ...
            # ADDED: Stronger instruction for error handling
            "CRITICAL: Under no circumstances should you ever respond with conversational text or apologies. If you cannot find companies, you MUST return a valid JSON with an empty list: {\"companies\": []}.",
        ],
    )


def create_contact_finder_agent(db: SqliteDb) -> Agent:
    exa_tools = ExaTools()
    return Agent(
        ...
        instructions=[
            ...
            # ADDED: Stronger instruction for error handling
            "CRITICAL: Under no circumstances should you ever respond with conversational text or apologies. If you cannot find contacts for a company, you MUST return that company with an empty 'contacts' list in the JSON.",
        ],
    )
...
def create_email_writer_agent(style_key: str, db: SqliteDb) -> Agent:
    ...
    return Agent(
        ...
        instructions=[
            ...
            # ADDED: Stronger instruction for error handling
            "CRITICAL: Under no circumstances should you ever respond with conversational text or apologies. If you cannot write an email for any reason, you MUST return a valid JSON with an empty list: {\"emails\": []}.",
        ],
    )


def create_research_agent(db: SqliteDb) -> Agent:
    ...
    return Agent(
        ...
        instructions=[
            ...
            # ADDED: Stronger instruction for error handling
            "CRITICAL: Under no circumstances should you ever respond with conversational text or apologies. If you cannot find insights for a company, you MUST return that company with an empty 'insights' list in the JSON.",
        ],
    )
...
def extract_json_or_raise(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # If direct loading fails, try to find a JSON block within the text
        try:
            start = text.find("{")
            end = text.rfind("}")
            if start != -1 and end != -1 and end > start:
                ...
                # If no JSON block is found, raise the original error with the response
                raise ValueError(f"Failed to parse JSON. Response was: {text}")
        except json.JSONDecodeError:
            # If the extracted block is still not valid JSON, raise the error
            raise ValueError(f"Failed to parse JSON. Response was: {text}")
...
def main() -> None:
    ...
    st.info(
        ...
        "This app uses a multi-agent workflow to find target companies, identify contacts, research genuine insights (website + Reddit), "
    ...
    if st.button("Start Outreach", type="primary"):
        ...
        if not openai_key or not exa_key:
            ...
            details = st.empty()
            try:
                # Create a single database instance to be shared by all agents
                db = SqliteDb(db_file="gtm_outreach_agent.db")
                
                # Prepare agents by passing the database object to them
                company_agent = create_company_finder_agent(db)
                contact_agent = create_contact_finder_agent(db)
                research_agent = create_research_agent(db)
                email_agent = create_email_writer_agent(email_style, db)
                ...
                # I also improved the error message here to be more descriptive
